...
Most of the time, this is due to the fact that EA's default install location is read only, which doesn't allow for Northstar to write logs properly. You can fix it by following the [default EA location section](installing-northstar/troubleshooting.md#cannot-write-log-file-when-using-northstar-on-ea-app)
