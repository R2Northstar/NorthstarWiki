...
VTOL has a built in browser for searching [Northstar's Thunderstore page](https://northstar.thunderstore.io/) (the website where most Northstar mods get uploaded to), located at the second tab "Browse" on the tab list. You will see the newest uploaded mods here by default, which you can change by applying filters with the "Sort" button (Note: The sorting is by default Low-High. You can change this by pressing the button with an arrow next to the "Sort" button, making it High-Low)
