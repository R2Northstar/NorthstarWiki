...
LatencyFleX's Vulkan layer can be installed with your package manager on supported distributions. On Arch it's available in [AUR](https://aur.archlinux.org/packages/latencyflex-git), and on Fedora it can be acquired from [Copr](https://copr.fedorainfracloud.org/coprs/kylegospo/LatencyFleX/).
...
For Steam and Steam Deck, run Northstar with the latest release of [NorthstarProton](https://github.com/cyrv6737/NorthstarProton/releases/latest).

### Reducing stuttering (Steam/NorthstarProton and Lutris/Wine)
...
DXVK-[_async_](https://github.com/Sporif/dxvk-async#improvements):

**Lutris:** download [**dxvk-async**](https://github.com/Sporif/dxvk-async/releases), extract and put it in `~/.local/share/lutris/runtime/dxvk` then type the name of the folder in `▲` -> `Configure` -> `Runner Options` -> `DXVK version`, to enable add `DXVK_ASYNC 1` to `System Options` -> `Environment variables`
###### *DXVK-async can also be installed for Lutris with [ProtonUp-Qt](https://davidotek.github.io/protonup-qt/)* ######

**Steam: DXVK-async is automatically installed and enabled if using the NorthstarProton runner.**
